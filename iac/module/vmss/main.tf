...
resource "null_resource" "provisioners" {
  ...
  provisioner "local-exec" {
    command = "sed -i '1s/^/${join("-", ["host", local.name_template, format("%02d", count.index + 1)])} ansible_ssh_host=${element(azurerm_public_ip.docker.*.fqdn, count.index)}\n/' ./provisioning/ansible/inventory/inventory"
  }
  provisioner "local-exec" {
